...
tf.keras.utils.get_custom_objects()['Sequential'] = Sequential
cap = cv2.VideoCapture("dataset/falltest.mp4")
...
with h5py.File("model.bak.h5", 'r') as f:
    ...
    model = tf.keras.models.load_model('model.bak.h5', custom_objects=custom_objects)
...
def detect(model, lm_list):
    global label
    lm_list = np.array(lm_list)  # 20, 132
    print(lm_list.shape)
    lm_list = np.expand_dims(lm_list, axis=0)   # 1, 20, 132
    # lm_list = np.reshape(lm_list, (1, -1))  # 1, 20*132
    ...
    if i > warm_up_frames:
        if results.pose_landmarks:
            ...
            if len(lm_list) == 20:
                ...
                t1.start()
                lm_list = []  # Xóa lm_list sau khi gửi đi để làm mới cho khung hình tiếp theo
            x_coordinate = []
            y_coordinate = []
            for lm in results.pose_landmarks.landmark:
                h, w, _ = frame.shape
...
df.to_csv(f'{label}.txt')  # Lưu dữ liệu vào file sau khi kết thúc
cap.release()
